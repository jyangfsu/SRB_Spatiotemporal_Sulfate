...
for iplot, par in enumerate(['T', 'pH', 'TDS', 'DO', 'K', 'Na', 'Ca', 'Mg', 'Cl', 'SO4', 'NO3', 'HCO3_mgL_tit', 'D', 'O18_gy', 'S34', 'O18_jz']):
    ...
    df = pd.concat([dfc1[(dfc1['Reach']=='Upper reaches') | (dfc1['Reach']=='Middle reaches') | (dfc1['Reach']=='Lower reaches' )][['Reach', par]],
                    dfc2[(dfc2['Reach']=='Upper reaches') | (dfc2['Reach']=='Middle reaches') | (dfc2['Reach']=='Lower reaches' )][['Reach', par]], 
                    dfc3[(dfc3['Reach']=='Upper reaches') | (dfc3['Reach']=='Middle reaches') | (dfc3['Reach']=='Lower reaches' )][['Reach', par]]])

    
    sns.boxplot(data=df, x='Reach', y=par, ax=axes[iplot], width=0.4, palette=palette, linewidth=1.0, 
    ...
    for i, reach in enumerate(['Lower reaches', 'Middle reaches', 'Upper reaches']):
        yy = df[par][df.Reach==reach].dropna()
        # Add some random "jitter" to the x-axis
        xx = np.random.normal(i, 0.04, size=len(yy))
        axes[iplot].scatter(xx, yy, color=palette[i], alpha=0.2, s=2)
        
    add_stat_annotation(axes[iplot], data=df, x='Reach', y=par, order=['Upper reaches', 'Middle reaches', 'Lower reaches'],
                        box_pairs=[('Upper reaches', 'Middle reaches'), ('Upper reaches', 'Lower reaches'), ('Middle reaches', 'Lower reaches')],
                        test='t-test_ind', text_format='star', 
                        loc='inside', verbose=2)
    
    axes[iplot].spines[['top', 'right']].set_visible(False)
    
    axes[iplot].plot([0, 1, 2], df.groupby('Reach').median()[par], 'k:', lw=1.5)
    
    axes[iplot].spines[['top', 'right', 'bottom']].set_visible(False)
    
    print('Number of samples:', df.shape[0]/3)
...
axes[7].set_ylabel('')
axes[7].set_xlabel('')
axes[7].set_ylim([0, 72])
axes[7].set_yticks([0, 36, 72])
...
axes[9].set_ylabel('')
axes[9].set_xlabel('')
axes[9].set_ylim([0, 340])
axes[9].set_yticks([0, 170, 340])
...
axes[11].set_ylabel('')
axes[11].set_xlabel('')
axes[11].set_ylim([0, 800])
axes[11].set_yticks([0, 400, 800])
...
axes[13].set_ylabel('')
axes[13].set_xlabel('')
axes[13].set_ylim([-12, 5])
axes[13].set_yticks([-12, -3, 5])
...
axes[12].set_xticks([0, 1, 2], ['Low.', 'Mid.', 'Upp.'], rotation=90)
axes[13].set_xticks([0, 1, 2], ['Low.', 'Mid.', 'Upp.'], rotation=90)
axes[14].set_xticks([0, 1, 2], ['Low.', 'Mid.', 'Upp.'], rotation=90)
axes[15].set_xticks([0, 1, 2], ['Low.', 'Mid.', 'Upp.'],  rotation=90)
...
plt.suptitle('(b) Spatial variation across river reaches', fontsize=11)
    
plt.subplots_adjust(hspace=0.5, wspace=0.4)


#plt.savefig('(b) Spatial variation of river water.jpg', dpi=300)